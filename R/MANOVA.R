...
#' @description This is a MANOVA rest for summarized data. This means that if we
...
#'     models.
#' @param groups list of sets of indices that identify models from a same group.
...
#'     are equal.
#' @return If boxM = TRUE and the p-value of the test is greater than 0.05, the
...
#'                control=list( maxiter = 1024, tol = 1e-12,
#'                minFactor = 10^-6 ))
#'
...
MANOVA <- function(obj, par.mat = NULL, cov.mat = NULL, sample.size = NULL,
  ...
  if ( !missing( obj ) ) {
    ...
    nls_lm_class <- all(sapply(obj, function(ob) inherits(ob, "nls.lm")))
    nls_class <- all(sapply(obj, function(ob) inherits(ob, "nls")))

    if (nls_class || nls_lm_class) {
      for( k in 1:n ) {
        ## To get the parameter variance-covariance matrix for each model
        Cov[[ k ]] <- unname( vcov( obj[[ k ]] ) )

        ## This generates a vector with the sample sizes used to estimate
        ##  each models
        m <- c( m, sum( summary( obj[[ k ]] )$df ) )

        ## This generated a matrix where each column contains the model
        ## parameters of each curve.
    ...
    if (lm_class || aov_class) {
      for( k in 1:n ) {
        ...
        # It generates a matrix where each column contains group means
        # or the model parameters of each group or linear model, respectively.
  ...
  if (is.null(par)) {
    stop("*** Non-informative models provided. \n",
  ...
  if( !missing( groups ) ) {
    ...
    for ( k in 1:z ) {
      ...
      # Paramater matrix for the groups
  ...
  df.h <- n - 1 # Degree of fredom for matrix H

  # "pooled" parameter sums of squares and cross product matrix E (SSCP):
  S <- mapply( function( x, y ) ( x - 1 )*y, m, Cov , SIMPLIFY = F )
  E <- Reduce( "+", S )
  df.e <- N - n # Degree of fredom for matrix E
  ...
  if( boxM ) # TEST FOR COVARIANCE MATRIX EQUALITY
  {
    ...
    M <- Reduce( "+", mapply( function(x,y) ( x - 1 )*log( det( y ) ),
                              m, Cov , SIMPLIFY = F ) )
    M <- df.e * log( det( Se ) ) - M
    # A transformation of M results in a statistic having a central chi-squared
    # distribution with degrees of freedom, v = (J - 1)(p + 1)p/2, under the
    # assumptions that the unit scores are independent and multivariate normal
    # in each population ([1] pag 41)
    if( mean( m ) == m[1] )
    {
      # If sample sizes are equal
      C <- 1 - ( (2*p^2 + 3*p - 1)*( n + 1 ) )/( 6*( p + 1 )*df.e )

    } else {
       C <- 1 - ((2*p^2 + 3*p - 1)/(6*(p+1)*(n-1))) * (sum(1/(m - 1)) - 1/df.e)}

    chisq <- C * M
    df <- ( n - 1 ) * ( p + 1) * p/2

    chi.pvalue <- pchisq( chisq, df, lower.tail = FALSE )
    # There are two limitations associated with the Box test. (pag 42) First, it
    # can be sensitive to multivariate nonnormality. Second, the degrees of
    # freedom for this test can often be very large, resulting in an extremely
    # sensitive/powerful test of the null hypothesis. Both of these limitations
    # could lead a researcher to question the validity of the multivariate test
    # on the vectors of means when the multivariate test is valid. As a result,
    # researchers often do not rely heavily on the results of the test but
    # rather rely on the robustness of the multivariate test on the equality of
  ...
  if( !inherits( W.inv, "try-error") )
  {
    ...
    if( n == 2 )
    {
      ...
      T2.H <- t(par[, 1] - par[, 2]) %*% Se.inv %*% (par[, 1] - par[, 2])
      T2.H <- m[1] * (m[2]/N) * T2.H
      ...
      # has a central F distribution with degrees of freedom v1 = p and v2 = dfe
      # - p + 1 = N - p - 1. This is true assuming independent units, bivariate
      # normality, equal population covariance matrices, and null hypothesis,
      # H0: par1 = par2, being true ([1] pag 40), where par1 & par2 are the
      # parameter vectors from model 1 & 2, respectivaly. "Hotelling T^2"
    ...
    if( boxM & n < 3 & chi.pvalue < 0.05 )
    {
      ...
      T2.u <- T2( W, par )
      f <- 1/(((b.1/T2.u )^2 )/(m[ 1 ] - 1) + (( b.2/T2.u )^2)/(m[ 2 ] - 1))
      v2 <- f - p + 1
      Y.F <- v2 * T2.u/( p * f )
    ...
    ####********** --- PILLAI' S TRACE  ***********####
    #- --------------------------------------------- -#
    # Pillai's trace. Also called Bartlett-Pillai-Hotelling trace criterion and
    # Bartlett - Pillai Criterion.
    # Let
    q <- ( abs( df.h - p ) - 1 )/2
    t <- ( df.e - p - 1 )/2
    ...
    # This statistic can be transformed to a statistic having an F distribution
    # with
    v1 <- r*( 2*q + r + 1 )
    v2 <- r*( 2*t + r + 1 ) # degrees of freedom. SPSS Statistics 17.0
                            # Algorithms (pag 401):
    ...
    if ( n > 3 ){# ([1] & [2])
      ...
      cond <- ( p^2 + df.h^2 - 5 )
      if ( cond > 0 ) b <- sqrt(( p^2 * df.h^2 - 4 )/cond )
      else b <- 1 # (3.21, [1] & [2] pag 7-8 )
    ...
    # Bartlett - Pillai criterion is about the same as that obtained using the
    # Wilks criterion.
  }else{
    T2.u <- NA
    Y.F <- NA
    ...
    V.F <- NA
    V.Fpval <- NA
    # warning( "The problem probably arises because of a high degree of
  ...
  if( n == 2)
  {
    if( boxM )
    {
      ...
      {
        report <- list()
        report$stats <- rbind( c( T2.H, T2.F, T2.Fpval ),
                              c( WLambda, WL.F, WL.Fpval ),
                              c( U, U.F, U.Fpval ),
                              c( V, V.F, V.Fpval ) )
        colnames( report$stats ) <- c( "Stat", "Fstat", "p.value" )
        rownames( report$stats ) <- c( "Hotelling T^2",
                                       "Pillai's trace",
                                       "Wilks lambda",
      ...
      report <- rbind( c( T2.H, T2.F, T2.Fpval  ),
                      c( WLambda, WL.F, WL.Fpval ) )
  ...
  if( n >= 3 )
  {
    ...
    {
      report <- list()
      report$stats <- rbind( c( U, U.F, U.Fpval ),
                            c( WLambda, WL.F, WL.Fpval ),
                            c( V, V.F, V.Fpval ))
      colnames( report$stats ) <- c( "Stat", "Fstat", "pvalue" )
      rownames( report$stats ) <- c( "Pillai's trace", "Wilks lambda",
                                     "Hotelling-Lawley' trace")

      report$Box.M <- data.frame( M, chisq, chi.pvalue )
      colnames( report$Box.M ) <- c( "Box M", "Chi^2", "chi.pvalue" )
    }else{
      report <- rbind( c( U, U.F, U.Fpval ),
                      c( WLambda, WL.F, WL.Fpval ),
                      c( V, V.F, V.Fpval ))
      colnames( report ) <- c( "Stat", "Fstat", "pvalue" )
      rownames( report ) <- c( "Pillai's trace", "Wilks lambda",
                               "Hotelling-Lawley' trace" )
